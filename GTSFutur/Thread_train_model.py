...
board=TensorBoard(log_dir="log/{}".format(time()),histogram_freq=1)




class Callback_get_value(Callback):
    
    def __init__(self):
         super(Callback_get_value,self).__init__()
         self.weight_arr = []
         print("creating")

    
    def on_epoch_end(self,logs="log/{}".format(time())):
        w = self.model.layers[len(self.model.layers)-1].get_weights()[0]
        self.weight_arr = np.append(self.weight_arr, [w])
            ...
            try:
                ...
                if e:
                    ...
                    print(
                        "***Not enought data in the input compared to the look back size. Data has size :" + str(
                            np.shape(x_train)) + "*****")
            #weight_dic=Callback_get_value()
            model.fit(x_train, y_train, epochs=nb_epochs, batch_size=self.nb_batch, verbose=self.verbose,callbacks=[board])
            print("model_trained")
        return model
